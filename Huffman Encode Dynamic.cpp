...
void write_encoded_file(FILE*& orig, FILE*& enco)
{
	char a;
	current_byte = 0;
	bit_counter = 0;
...
int main(int argc, char** argv)
{
	...
	if (argc != 3)
	{
		fprintf(stderr, "./huffman_encode_dyamic.out <text file name> <encoded file name>");
	}
	std::string text, encoded;
	text = argv[1];
	encoded = argv[2];

	orig = fopen(text.c_str(), "r");
	encode = fopen(encoded.c_str(), "wb");
	...
	freequeue(buff.top());
	write_freq_file();
	write_encoded_file(orig, encode);
